...
const CreateRecipe = () => {
  ...
  const [tags, setTags] = useState<string[]>([]); // This will now hold selected tags
  // Removed newTag state as we are using predefined tags now

  const allowedUnits = [
    "gram", "kilogram",
    "milliliter", "liter", "tablespoon(s)", "teaspoon(s)",
    "piece(s)", "slice(s)"
  ];
  ...
  // --- NEW: Toggle tag selection function ---
  const toggleTag = (tagToToggle: string) => {
    if (tags.includes(tagToToggle)) {
      setTags(tags.filter(tag => tag !== tagToToggle)); // Remove tag if already selected
    } else {
      setTags([...tags, tagToToggle]); // Add tag if not selected
    }
  };

  // Removed old addTag and removeTag functions that were for free text input
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formDataToSend.append('tags', JSON.stringify(tags)); // tags now contains selected predefined tags
    ...
    console.log("Sending recipe data to backend (via FormData):", Object.fromEntries(formDataToSend.entries()));
    ...
    try {
      ...
      const response = await axios.post<IRecipe>("http://localhost:8080/recipes", formDataToSend, {
        headers: {
      ...
      // Always set createdRecipeId if available
          ...
          <div className="flex gap-2">
            ...
            <Button 
              ...
              onClick={() => {
                setShowUnmatchedModal(false);
                if (createdRecipeId) {
                  navigate(`/recipes/${createdRecipeId}/edit`);
                } else {
                  navigate("/recipes");
                }
              }}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <Select value={formData.difficulty} onValueChange={(value) => handleSelectChange("difficulty", value)}>
                    ...
                    <SelectContent>
                      {/* REMOVED the <SelectItem value=""> component to resolve Radix UI error */} 
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-wrap gap-2">
                {predefinedTags.map((tag, index) => (
                  <Badge
                    key={index}
                    variant={tags.includes(tag) ? "default" : "secondary"} // Highlight if selected
                    className={`cursor-pointer ${
                      tags.includes(tag)
                        ? "bg-emerald-600 text-white hover:bg-emerald-700" // Selected style
                        : "bg-emerald-100 text-emerald-700 hover:bg-emerald-200" // Unselected style
